...
import ReferenceList from './DietReferenceList';
import ReferenceForm from './DietReferenceForm';

export default function DietReference() {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-2 p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Diet Reference</h1>
        {isAdmin && (
          <button
            onClick={toggleForm}
      ...
      {isAdmin && showForm && (
        <div className="mb-6">
          <ReferenceForm onAdd={() => setShowForm(false)} />
        </div>
      )}

      {/* Reference list with filter */}
      <ReferenceList searchTerm={searchTerm} />
    </div>
  );
}
